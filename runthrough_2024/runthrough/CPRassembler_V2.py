...
def main():
    ...
    print('booting up...\n')
    loan_data = pd.read_csv('raw_data/processed_data_2024.csv')
    ...
    loan_data['CodeText'] = loan_data['CodeText'].fillna('')
    loan_data['Code'] = loan_data['Code'].fillna(-1)
    ...
    # Ask user for custom margin buckets
        ...
        while user_initiate == 0:
            print(f"Current Loan Count: {combo_slice.shape[0]}")    
            ...
            if col == 'Code':
                bins = input("Enter NAICS codes seperated by a comma\n")
                bins = [int(e) for e in bins.split(',')]
                print(f"hotels: {combo_slice[combo_slice['Code'].isin(bins)].shape[0]}")
                ...
                print(f"combo_slice: {combo_slice.shape[0]}, combo_condition: {combo_condition}, inverse: {inverse_opt}")
                ...
                # print(combo_slice[f'CustomBuckets_{choice}'].value_counts())
